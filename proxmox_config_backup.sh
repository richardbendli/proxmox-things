...
# Default value can be overridden by setting environment variable before running proxmox_config_backup.sh
#   example: export BACK_DIR="/mnt/pve/media/backup"
#   or
#   example: BACK_DIR="." ./proxmox_config_backup.sh
...
function check-num-backups {
    if [[ $(find "${_bdir}" -name "*${_HOSTNAME}*_*.tar.gz" -type f | wc -l) -ge $MAX_BACKUPS ]]; then
